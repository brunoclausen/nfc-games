...
- The AppImage copies itself to `~/Applications` and writes a user systemd
  unit, which it enables so watch starts at login. Disable it with
  `systemctl --user disable --now nfc-games.service`. Nothing runs as root
  except the udev install.
